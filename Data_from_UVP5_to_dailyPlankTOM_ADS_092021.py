#colocalisation of UVP5 measurements with daily PlankTOM12
import numpy as np
import xarray as xr
import pandas as pd
...
for key, item in data_all_Tara_date:
   ...
   if year > 2008 and year < 2014:
    ...
    if t == 0: 
       ...
       print(filename_ptrc)
    if year_before != year:
       filename = 'TOM12_JG_ASG3/ORCA2_1d_'+str(year)+'0101_'+str(year)+'1231_grid_T.nc'
       ds = xr.open_dataset(filename)
       filename_ptrc = 'TOM12_JG_ASG3/ORCA2_1d_'+str(year)+'0101_'+str(year)+'1231_ptrc_T.nc'
       ds_ptrc = xr.open_dataset(filename_ptrc)
       filename_chl = 'TOM12_JG_ASG3/ORCA2_1d_'+str(year)+'0101_'+str(year)+'1231_diad_T.nc'
       ds_chl = xr.open_dataset(filename_chl)
       year_before = year
       print(filename_ptrc)
...
#save dataframe
Data_all.to_csv('Data_RKiko_Back_PlankTOMDaily_bottom_21102021.csv')
