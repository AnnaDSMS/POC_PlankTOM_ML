#colocalization of UVP5 data with PlankTOM12
import numpy as np
import xarray as xr
import pandas as pd
...
#concatinate two dataframe of UVP5 measurements
data_all_1 = pd.concat([data_20082010, data_20112013],ignore_index=True, sort=False).sort_values(['Date_Time', 'Latitude','Longitude','Pressure [dbar]']).reset_index()

#creation of columns for year, month and day
data_all_1[['date','time']] = data_all_1.Date_Time.str.split("T",expand=True,)
...
#positions of UVP5 measurements
Data_position = data_all_1[['Latitude','Longitude','year','month','day']]
Data_position['coordinates'] = list(zip(Data_position.Latitude, Data_position.Longitude))
...
for key, item in data_all_Tara_date:
   ...
   if year > 2008 and year <2014:
    ...
    for key1, item1 in data_all_Tara_date_coord:
        ...
        lat_array = data_all_Tara_date_coord.get_group(key1)['Latitude'].values
        lon_array = data_all_Tara_date_coord.get_group(key1)['Longitude'].values
        if lat_array.size > 1:
